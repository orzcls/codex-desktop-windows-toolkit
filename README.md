...
这个仓库把 Windows 版 Codex 桌面能力补全经验整理成一个可发布、可审计、可复用的工程项目。它不修改程序二进制文件，也不复制用户本机的私密运行态；它提供的是一组安全模板、验证脚本、运行手册和长任务状态文件，让 Browser、Computer Use、插件、MCP 与 `/goal` fallback 可以在 Windows 上更稳定地协同工作。
...
第五，发布资产必须脱敏。真实 API key、auth state、cookies、SQLite 日志、浏览器会话和桌面截图不得进入 Git。
...
README.md       中文说明
README.en.md    英文说明
docs/           Browser、Computer Use、Goal、Security、Release 文档
...
检查当前私有 Codex 配置时，不要直接复制 `.codex`。使用导出脚本写到被 Git 忽略的 `artifacts/`：
...
- API key、OAuth token、cookies、浏览器登录态。
- `.credentials.json`、`auth.json`、Codex SQLite 数据库和 WAL/SHM 文件。
- 原始 `.codex/config.toml` 中包含真实环境变量的版本。
- 含有个人桌面内容的截图。

校验脚本会扫描常见密钥模式，但它只是防线之一。推送 GitHub 前仍然需要人工查看 `git diff --cached`。
...
它也适合作为一个模板：把个人环境里的经验转化为公共仓库时，应该保留结构、脚本和验证方法，剥离密钥、会话和一次性缓存。
